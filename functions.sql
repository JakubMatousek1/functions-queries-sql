...
MAX(CASE WHEN column_name = 'some_value' THEN 1 ELSE 0 END) OVER (PARTITION BY another_column)          --> non-curable flag - once hit it stays - 0,0,0,1,1,1,.....1
SUM(amt) OVER (PARTITION BY column_nm ORDER BY date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)   --> Running total (Cumulative sum)
